  ...
  static SpeedtestResult runSpeedTest(DistanceUnit distanceUnit, boolean testDownload, boolean testUpload,
    ...
    if (distanceUnit != null) {
      try {
        ...
        ConfigSetting configSetting = ConfigSettingsService.requestSetting();
        if (generateOutput) {
        ...
        List<Server> serverList = ServerSettingsService.requestServerList(configSetting.getDownload().getThreadsPerURL());
        Map<Double, Server> closestServers = ServerSettingsService.findClosestServers(configSetting.getClient().getLat(),
        ...
        LatencyTestResult fastestServer = LatencyService.getFastestServer(closestServers, distanceUnit);
        if (generateOutput) {
          System.out.printf("Hosted by %s (%s) [%,.2f %s]: %,.2f ms\n",
              fastestServer.getServer().getSponsor(), fastestServer.getServer().getName(),
              fastestServer.getDistance(), distanceUnit.getAbbreviation(), fastestServer.getLatency());
        }
        ...
        if (testDownload) {
          ...
          downloadResult = DownloadService.testDownload(fastestServer.getServer().getUrl(),
        ...
        if (testUpload) {
          ...
          uploadResult = UploadService.testUpload(fastestServer.getServer().getUrl(),
        ...
        if (generateShareURL && uploadResult != null && downloadResult != null) {
          shareURL = ShareUrlService.createShareURL(fastestServer.getServer().getId(), fastestServer.getLatency(),
        ...
        return new SpeedtestResult(System.currentTimeMillis(), fastestServer, downloadResult, uploadResult, shareURL);
      } catch (Exception e) {
        if (generateOutput) {
